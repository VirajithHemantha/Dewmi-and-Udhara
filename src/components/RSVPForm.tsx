...
import { CheckCircle, XCircle, Loader2, Heart, Sparkles } from 'lucide-react';
...
export const RSVPForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const normalizedGuests = rsvpStatus === 'accepted' ? parseInt(formData.guests, 10) : 0;
    ...
    try {
      await submitToGoogleSheet('rsvp', {
        ...
        dietaryNotes: rsvpStatus === 'accepted' ? formData.dietaryNotes : '',
          ...
          <AnimatePresence mode="wait">
            {status === 'success' ? (
              ...
              >
                ...
                <h3 className="text-4xl font-display text-stone-800 mb-4 tracking-tight drop-shadow-sm">
                  {rsvpStatus === 'accepted' ? 'With Gratitude' : 'We Understand'}
                </h3>
              ...
              >
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button
                      ...
                      className={`flex items-center justify-center gap-2 py-4 rounded-full border-2 font-sans text-[11px] uppercase tracking-[0.2em] font-bold transition-all duration-300 ${
                        rsvpStatus === 'accepted'
                          ? 'bg-emerald-500 border-emerald-500 text-white shadow-[0_6px_20px_rgba(16,185,129,0.35)]'
                          : 'bg-white/70 border-stone-200/60 text-stone-500 hover:border-emerald-400 hover:text-emerald-600'
                      }`}
                    >
                    ...
                    <button
                      ...
                      className={`flex items-center justify-center gap-2 py-4 rounded-full border-2 font-sans text-[11px] uppercase tracking-[0.2em] font-bold transition-all duration-300 ${
                        rsvpStatus === 'declined'
                          ? 'bg-rose-400 border-rose-400 text-white shadow-[0_6px_20px_rgba(251,113,133,0.35)]'
                          : 'bg-white/70 border-stone-200/60 text-stone-500 hover:border-rose-400 hover:text-rose-500'
                      }`}
                    >
                ...
                <div className="pt-2">
                  <button
                    disabled={status === 'loading' || !rsvpStatus}
                    type="submit"
                    className="w-full bg-stone-800 text-brand-white py-5 rounded-full font-sans tracking-[0.3em] font-bold text-[11px] uppercase hover:bg-stone-900 transition-all duration-300 shadow-[0_10px_20px_rgba(0,0,0,0.15)] hover:shadow-[0_15px_30px_rgba(0,0,0,0.25)] active:scale-[0.98] flex items-center justify-center gap-3 disabled:opacity-40 disabled:cursor-not-allowed"
                  >
                    {status === 'loading' ? (
